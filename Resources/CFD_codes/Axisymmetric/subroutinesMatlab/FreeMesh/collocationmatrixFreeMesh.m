function [dPhidx,dPhidy,dPhidxx,dPhidyy,C]=collocationmatrixFreeMesh(la,f1,g1)
...
for n=1:N
    ...
    ptdx     = Ed(1,1:M)  .* ra(1).^dE(1,:)  .* za(1).^E(2,:);  % [0,1,0,za(1),2*ra(1),0];
    ptdy     = Ed(2,1:M)  .* ra(1).^E(1,:)  .* za(1).^dE(2,:);  % [0,0,1,ra(1),0,2*za(1)];
    ptdxx    = Ed2(1,1:M) .* ra(1).^d2E(1,:) .* za(1).^dE(2,:); % [0,0,0,0,2,0];
    ptdyy    = Ed2(2,1:M) .* ra(1).^dE(1,:) .* za(1).^d2E(2,:); % [0,0,0,0,0,2];
