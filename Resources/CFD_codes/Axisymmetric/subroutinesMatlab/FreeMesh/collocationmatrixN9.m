function [dx,dy,dxx,dyy]=collocationmatrixN9(la,f1,g1)
...
n=length(la(:,1));
 ...
 [duxva(l,:),duyva(l,:),duxxva(l,:),duyyva(l,:)]=collocationmatrixN9a(rbb,zbb);
 end

 %motamos matrices
 duxva=reshape(duxva,n*9,1);
 duyva=reshape(duyva,n*9,1);
 duxxva=reshape(duxxva,n*9,1);
 duyyva=reshape(duyyva,n*9,1);
 ja1=reshape(la,n*9,1);
% ja1=[la(:,1);la(:,2);la(:,3);la(:,4);la(:,5);la(:,6)];
 ia=[1:n,1:n,1:n,1:n,1:n,1:n,1:n,1:n,1:n];
