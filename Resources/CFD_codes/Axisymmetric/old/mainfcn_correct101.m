function mainfcn_correct101(Ib_fe)
...
%Ib_fe    = 1;          % Coils current intensity (A)
...
lstart   = 1; % 0 to start meshing from scratch, 1 to start with name_prev_sol solution
lcompute = 0; % Compute mesh
filesave = 1; % Save (1) or don't save (0) solution

% Previous solution
files    = dir(['./soluciones/DYT-21-Apr-2021_Ib*_zeta1,2571e-05tau5,5194e-07nrA',num2str(nrA),'nzA',num2str(nzA),'nrB41nrC21nrE61nzE',num2str(nzE),'.mat']);
for i = 1:length(files)
    namefile = files(i).name;
    zetapos  = strfind(namefile,'zeta');
    Ibpos  = strfind(namefile,'Ib');
    Ib_feN0(i)= str2double(namefile(Ibpos+2:zetapos-2));
end
[~,fileid]    = min(abs(Ib_feN0/Nturns - Ib_fe));
name_prev_sol = ['soluciones/DYT-21-Apr-2021_Ib',num2str(Ib_feN0(fileid)),...
    '_zeta1,2571e-05tau5,5194e-07nrA',num2str(nrA),'nzA',num2str(nzA),'nrB41nrC21nrE61nzE',num2str(nzE),'.mat'];
...
if (lstart==0)
    ...
    % parameters
    pa=[rho;eta0;gamma;theta;H1;R;H2;aM;bM;cM;dM;eM;Ib;Sb;m1;mz1;m2;mz2;Mmagnet;zeta;tau;VF;0;0;RminD;RmaxD;ZminD;ZmaxD;Rout;zout];
    ...
    % NEW: Pointers CoilD, CoilD1, CoilD_contour
...
%         dd0zD, dd0zzD,ZminD,ZmaxD,RminD,RmaxD)
    ...
    if ksave==1 && filesave ==1
        clear a b
        % Case name
        finame = ['soluciones/DYT-21-Apr-2021_Ib' po2com(Ib) '_zeta' po2com(zeta),...
            'tau' po2com(tau) 'nrA' num2str(nrA)     'nzA' num2str(nzA)  'nrB' ...
            num2str(nrB) 'nrC' num2str(nrC) 'nrE' num2str(nrE)    'nzE' num2str(nzE)];
