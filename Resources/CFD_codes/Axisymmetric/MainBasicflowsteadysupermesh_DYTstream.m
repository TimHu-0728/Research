...
H1    = 0.2; % Height of the dry container
nzB   = nzA; % Points in the r
nrB   = 31;  % Points in the z

% Block C (Air over container)
H2    = 0.6; % Height of the domain
nzC   = nzA; % Points in z
nrC   = 31;  % Points in r

% Block E (Air under container)
zout  = -0.6;  % Lower end of the domain
nzE   = nzA; % Points in z
nrE   = 61;  % Points in r
...
Rout     = 0.6;   % External radius of the domain
d_nz     = 7;  % Density of points per cm of D domain
nz1      = 4*floor((100*d_nz * (RminD-R))/2)+1;     % Pointers before the coil (in the radial direction)
nzbobina = 3*floor((100*d_nz * (RmaxD-RminD))/2)+1; % Pointers inside the coil
nz2      = 2*floor((100*d_nz * (Rout-RmaxD))/2)+1;  % Pointers after the coil
...
Ib_fe    = 5;           % Coils current intensity (A)
...
leq      = 0; % 1 to symbolic functions and construction of the matrix, 0 otherwise
lb       = 5; % Specifies block to be compiled (1:A, 2:B, 3:C, 4:D, 5:E)
lstart   = 0; % 0 to start meshing from scratch, 1 to start with name_prev_sol solution
lcompute = 1; % Compute mesh
name_prev_sol = 'soluciones/DYT-14-Mar-2021_Ib0zeta1,2571e-05tau1,2571e-05nrA71nzA71nrB31nrC31nrE61nzE71_good.mat';
...
list_var_D = {'phiD','psiD' 'FD' 'GD'};%phiB is the streamfunction for H ellipitical mesh
...
if (lstart==0)
    ...
    %[z0A,dz0A,dzz0A]=finitas4ordentotal(nzA,1);  %direction in s(r)
    %[r0A,dr0A,drr0A]=finitas4ordentotal(nrA,1); %%direction in eta (z)
    
    [z0A,dz0A,dzz0A]=finitasdoblegood(nzA,1);
    [r0A,dr0A,drr0A]=finitasdoblegood(nzA,1);
    
    %[z0A,dz0A,dzz0A]=finitas2(nzA,1);                %%%%%%%% 18/09/2020
    %[r0A,dr0A,drr0A]=finitas2(nzA,1);                %%%%%%%% 18/09/2020
    ...
    ntB=nrB*nzB;
    %[z0B,dz0B,dzz0B]=finitas2(nzB,1);  %direction in s(r)
    %[r0B,dr0B,drr0B]=finitas2(nrB,1); %%direction in eta (z)
    [r0B,dr0B,drr0B]=Chevitanh2th_i(nrC,0,1,3);                %%%%%%%% 10/03/2021
    ...
    %[z0C,dz0C,dzz0C]=finitas2(nzA,1);
    [z0C,dz0C,dzz0C]=finitasdoblegood(nzC,1);                %%%%%%%% 18/09/2020
    [r0C,dr0C,drr0C]=finitas2(nrC,1);    
    ...
    % Chevitanh2th(nr1,Rin, Rout,a)
    [r0E,dr0E,drr0E]=finitas2(nrE,1);
    
    
    %        plot(r0E,r0E,'x')
    %     hold on
    [r0E,dr0E,drr0E]=Chevitanh2th(nrE,0,1,3);
    %       plot(r0E,r0E,'o')
    ...
    %getting pointers and all and creating D
    ...
    % parameters
    pa=[rho;eta0;gamma;theta;H1;R;H2;aM;bM;cM;dM;eM;Ib;Sb;m1;mz1;m2;mz2;Mmagnet;zeta;tau;VF;0;0];
    ...
    load(name_prev_sol, '-regexp','^(?!Ib_fe$|Nturns$|dt$|Nsteps$|miniter$|maxerror$|tiempo$|theta_fe$|gamma_fe$|hmin$|eta_fe$|zeta_fe$|tau_fe$|Mmagnet_fe$).');
...
%         velocities(za,ra,zb,rb,zc,rc,zd,rd,ze,re,psiA,psiB,psiC,psiD,psiE)
    ...
    mass;
    %plot(0.1*ll,gA(nrA,1)-gaxisA(nrA,1),'r*')
    %plot(fA(nrA,:),gA(nrA,:)-gaxisA(nrA,:))
    %drawnow
    % % % x0=eval([order ']']);
    order='[';
    for j=1:nbl
    ...
    if ksave==1 && filesave ==1
        clear a b
        % Case name
        finame = ['soluciones/DYT-',date,'_Ib' po2com(Ib) 'zeta' po2com(zeta),...
            'tau' po2com(tau) 'nrA' num2str(nrA)     'nzA' num2str(nzA)  'nrB' ...
            num2str(nrB) 'nrC' num2str(nrC) 'nrE' num2str(nrE)    'nzE' num2str(nzE) '_good'];
        finame = ['soluciones/DYT-11-Nov-2020_Ib' po2com(Ib) 'zeta' po2com(zeta),...
            'tau' po2com(tau) 'nrA' num2str(nrA)     'nzA' num2str(nzA)  'nrB' ...
            num2str(nrB) 'nrC' num2str(nrC) 'nrE' num2str(nrE)    'nzE' num2str(nzE) '_good'];
