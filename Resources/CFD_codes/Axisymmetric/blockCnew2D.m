...
FCClinea13(1)=phi; % -phi is in CD  (variable 1)
FCClinea13(2)=f-R;
FCClinea13(3)=psi; % -phi is in CD  (variable 1)
%conexion with D
FDClinea13(1)=dphir; %-Br is in Block DD (line 9) (variable 1)
FDClinea13(2)=dpsir; %-Br is in Block DD (line 9) (variable 1)
FDClinea13(3)=r;
FDClinea13(4)=z;
FDClinea13(5)=0;
...
%VertexDBC
%D
FVertexDBCC(1)=phi; %DBCD
FVertexDBCC(2)=psi;
FVertexDBCC(3)=r; %DBCD
FVertexDBCC(4)=z;
FVertexDBCC(5)=0;


FVertexCDB(1)=dphir; %v;  %look at CDBD
FVertexCDB(2)=f-R;
FVertexCDB(3)=dpsir; %v;
...
%conexion with B
 FCClinea12(1)=-dphiz;%  
 FCClinea12(2)=-f;% 
 FCClinea12(3)=-dpsiz; %-Br is in Block DD (line 9) (variable 1)
...
% Psi exact (2 is coil -c- or magnet -m-)


FCClinea6_c(1)=psi-psi_c;
FCClinea6_c(2)=df0r_res;%
FCClinea6_c(3)=phi-phid_c;

FCClinea6_m(1)=psi-psi_m;
FCClinea6_m(2)=df0r_res;%
FCClinea6_m(3)=phi-phid_m;
...
for k=1:NVAR
    ...
    dFCC(k,:)=jacobian(FCC(k),x);
    dFCClinea12(k,:)=jacobian(FCClinea12(k),x);
    dFCClinea6_c(k,:)=jacobian(FCClinea6_c(k),x);
    dFCClinea6_m(k,:)=jacobian(FCClinea6_m(k),x);
    dFCClinea13(k,:)=jacobian(FCClinea13(k),x);
    dFCClinea5(k,:)=jacobian(FCClinea5(k),x);
    dFVertexCDB(k,:)=jacobian(FVertexCDB(k),x);
end

matlabFunction(dHrdz-dHzdr,'file',  [path_jacobian 'curlH_C.m'],'vars',{z1,r1,x,pa});
matlabFunction(dphir,dphiz,dphirr,dphizz,dphirz,dpsir,dpsiz,dpsirr,dpsizz,dpsirz,...
    'file',[path_jacobian 'potentialsCC.m'],'vars',{z1,r1,x,pa});
matlabFunction(FVertexCDB,dFVertexCDB,'file',[path_jacobian 'equationFVertexCDB.m'],'vars',{z1,r1,x,pa});
matlabFunction(Hra,Hza,'file',  [path_jacobian 'MagneticfieldCCa.m'],'vars',{z1,r1,x,pa});

matlabFunction(Hr,Hz,'file',  [path_jacobian 'MagneticfieldCC.m'],'vars',{z1,r1,x,pa});
matlabFunction(FCC,dFCC,'file',  [path_jacobian 'equationFCC.m'],'vars',{z1,r1,x,pa});
matlabFunction(FCClinea12,dFCClinea12,'file',[path_jacobian 'equationFCClinea12.m'],'vars',{z1,r1,x,pa});
matlabFunction(FCClinea6_c,dFCClinea6_c,'file',[path_jacobian 'equationFCClinea6_c.m'],'vars',{z1,r1,x,pa,phid_c});
matlabFunction(FCClinea6_m,dFCClinea6_m,'file',[path_jacobian 'equationFCClinea6_m.m'],'vars',{z1,r1,x,pa});
matlabFunction(FCClinea13,dFCClinea13,'file',[path_jacobian 'equationFCClinea13.m'],'vars',{z1,r1,x,pa});
matlabFunction(FCClinea5,dFCClinea5,'file',[path_jacobian 'equationFCClinea5.m'],'vars',{z1,r1,x,pa});


 for k=1:NVB
 dFBClinea12(k,:)=jacobian(FBClinea12(k),x);
 end
 matlabFunction(FBClinea12,dFBClinea12,'file',[path_jacobian 'equationFBClinea12.m'],'vars',{z1,r1,x,pa});
% 
...
for k=1:NVD
dFDClinea13(k,:)=jacobian(FDClinea13(k),x);
dFVertexDBCC(k,:)=jacobian(FVertexDBCC(k),x);
end
matlabFunction(FDClinea13,dFDClinea13,'file',[path_jacobian 'equationFDClinea13.m'],'vars',{z1,r1,x,pa});
matlabFunction(FVertexDBCC,dFVertexDBCC,'file',[path_jacobian 'equationFVertexDBCC.m'],'vars',{z1,r1,x,pa});
