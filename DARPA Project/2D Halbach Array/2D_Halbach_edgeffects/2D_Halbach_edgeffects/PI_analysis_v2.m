...
for j = 1:length(lambda)
    ...
    w_vec0           = ones(N_coil_inf,1)*w(j);
    w_vec0(2:2:end)  = h(j);

    %height input for calculating B field
    h_vec0           = ones(N_coil_inf,1)*h(j);
    h_vec0(2:2:end)  = w(j);
    ...
    for k = 1:size(r0,1)
        Plot_magnets(theta0(k),w_vec0(k),h_vec0(k),r0(k,:),1,1);
    end
    xlim([-0.3 0.3])
    axis equal
    ...
    % % Calculating short Halbach Array potential field
    % for i = 1:length(desired_len)
    %     Mm          = (1.48/mu0)*ones(N_coil(i,j),1);                    % [A/m]   Magnetization
    %     Ke          = Mm;                                                % [A/m]   Sheet current
    % 
    %     %magnet placement (x,y coordinates)
    %     N_side_diff = (N_coil_inf - N_coil(i,j))/2;
    %     r           = r0((N_side_diff+1):(end-N_side_diff),:);
    % 
    %     %magnet magnetization direction
    %     theta       = theta0((N_side_diff+1):(end-N_side_diff),:);
    % 
    %     %width input for calculating B field
    %     w_vec            = w_vec0((N_side_diff+1):(end-N_side_diff),:);
    % 
    %     %height input for calculating B field
    %     h_vec            = h_vec0((N_side_diff+1):(end-N_side_diff),:);
    % 
    %     % Calculates B fields [T]
    %     [Bx,By]         = calculatingB(mu0,Ke,h_vec,w_vec,X,Y,theta,r);         % B field for contour plot
    %     B               = sqrt(Bx.^2+By.^2);
    %     Hx              = Bx / mu0;                                                 %obtaining X magnetic field from B field
    %     Hy              = By / mu0;                                                 %obtaining Y magnetic field from B field
    %     H_norm          = sqrt(Hx.^2+Hy.^2);
    ...
    %     PI_g        = g*(Y_above.* cos(theta_0) - X_above.* sin(theta_0));
    %     PI          = PI_g+PI_m;
    %     deviation_y = (PI-PI_ref)./nabla_PI_ref_y;
    % 
    % 
    %     % Plot PI, deviation
    %     figure
    %     %subplot(3,3,j+3*(i-1))
    %     % contourf(X_above,Y_above,log10(1e6 * abs(deviation_y)), [-2:1:3])
    %     contourf(X_above,Y_above,PI-PI_ref)
    ...
    %     % Plot the magnetized material shapes
    %     for k = 1:size(r,1)
    %         Plot_magnets(theta(k),w_vec(k),h_vec(k),r(k,:),1,1);
    %     end
    %     axis equal
    % 
    %     % quiver(X,Y,Bx,By)
