...
% Sets meshgrids [User Input]
gridsz   = 1000;                                             % Grid size for contour plot
gridsz_q = 50;                                               % Grid size for quiver plot
range    = 2.5*max(r0(:,1));                                               % Range of the plot

[X,Y]    = meshgrid(linspace(-range,range,gridsz),linspace(-range,range,gridsz)+0.005);
[Xq,Yq]  = meshgrid(linspace(-range,range,gridsz_q),linspace(-range,range,gridsz_q)+0.005);
...
Fk_x     = mu0 * (dot(M_vec,cat(3,gradient(Hx),zeros(size(Hx))),3));
Fk_y     = mu0 * (dot(M_vec,cat(3,zeros(size(Hy)),gradient(Hy)),3));
Fk       = sqrt(Fk_x.^2+Fk_y.^2);
Fk       = Fk/rho/9.81;
...
figure
levels = linspace(0,1.5,15);
contourf(X,Y,B,levels)
...
contourf(X,Y,Fk)
c              = colorbar('FontSize',22);
c.Label.String = '(Kelvin Body Force Strength (N))';
hold on
