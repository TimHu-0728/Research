...
% Ms      = Ms_rho_cfit(rho_Chi0_cfit(chi0)) * 795.7747;                          % Saturation Magnetization Found corresponding to chi0 [A/m]
% % Ms      = Ms_rho_cfit(rho)/(1000*4*pi*1e-7)
% g       = 9.806%1.62%1e-6;%9.806;                                                             % Gravitational Acceleration [m/s^2]
...
% Ms      = Ms_rho_cfit(rho)/(1000*4*pi*1e-7)
g       = 9.81%9.806%1e-6%9.806;                                                             % Gravitational Acceleration [m/s^2]
mu0     = 4*pi*1e-7;                                                         % Vaccum Pemeability [N/A^2]
gamma   = 3*chi0/Ms; 
sigma   = 0.025;                                                             % Interfacial tension [N/m]
...
opts.PopulationSize         = 1000%6000*2; %200 by default, 802 is optimal (perfect case)
...
%% result from 8 coils to optimize 9 coils
%IC from 8 coils
% opts.InitialPopulationMatrix     = [0.588383981547886	log(485165170.933837)	-1.72260757993145	2.79997067908711	log(359316992.752377)	3.49641988826019	14.9285177011837	log(408364128.717797)	2.83982081910316	14.5374926914915	log(485165186.442234)	-1.12693906540718	7.98872020660050	log(279506952.816751)	4.01914953252289	15.0000491889102	log(485165124.793482)	-1.23842545731565	5.27514960315921	log(5534.18188877186)	-5.99997145166952	24.9999999692797	log(25044508.9932203)	-5.99999921593430	24.9999999732982];

% opts.InitialPopulationMatrix     = [0.588383981547886	485165170.933837	-1.72260757993145	2.79997067908711	359316992.752377	3.49641988826019	14.9285177011837	408364128.717797	2.83982081910316	14.5374926914915	485165186.442234	-1.12693906540718	7.98872020660050	279506952.816751	4.01914953252289	15.0000491889102	485165124.793482	-1.23842545731565	5.27514960315921	5534.18188877186	-5.99997145166952	24.9999999692797	25044508.9932203/2	-5.99999921593430	24.9999999732982 25044508.9932203/2	-5.99999921593430	24.9999999732982];
% opts.InitialPopulationMatrix(24) = InitialPopulationMatrix(24)/2;
% opts.InitialPopulationMatrix(26:28) = [x(23) x(24)/2 x(25)]
%% Summoning GA
...
% Plotting
[actual_contour_x_coordinate, actual_contour_y_coordinate,PI_diff,max_error_in_nm] = result_plots(x, r_flat, I, rz, N_count,parameters);                                                                             %This function was created to localize the plots somewhere other than the main code.
