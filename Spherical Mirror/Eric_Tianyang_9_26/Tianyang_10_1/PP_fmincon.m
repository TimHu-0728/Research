...
yy     = surf_func(xx);
r_upper_lim = 30;                                                               %upper limit for radius in the optimization, in meters
...
I_upper_lim = 30;                                                               %upper limit for ln(current) (measured in ln(Amperes))
...
function [c,ceq] = nlcon(y)
    ...
    sigma   = 0.01;                       % Interfacial tension [N/m]
    r_flat  = 15;
    N_count = 31;                         % Number of points along the surface (Rosensweig Instability)
    step    = 0.0001;
    ...
    % Constrain for coil being under the mirror surface
    c(1:IN)=[y(3)-(30 - sqrt(900 - y(4) .^ 2));
             y(6)-(30 - sqrt(900 - y(7) .^ 2))
             y(9)-(30 - sqrt(900 - y(10) .^ 2))
             y(12)-(30 - sqrt(900 - y(13) .^ 2))];

    % Constrain for Material Magnetization smaller than critical magnetization along the surface (Avoid Rosensweig Instability)
    c(IN+1:end) = M-Mc;
